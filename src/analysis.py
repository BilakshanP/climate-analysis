...
def _fetch_api(lat, lon, start, end):
    ...
    for attempt in range(4):
        ...
        if r.status_code == 429 and attempt < 3:
            wait = delays[attempt]
            print(
                f"    ⚠ Rate limited (429). Retrying in {wait}s... (attempt {attempt + 2}/4)"
            )
            time.sleep(wait)
            continue
        r.raise_for_status()
...
def _load_city(label, lat, lon, start_year, end_year):
    ...
    if not gaps:
        ...
        print(
            f"  {label} — {'cached [' + fmt_ranges + ']' if existing else 'no cache'}"
        )
        for gs, ge in gaps:
    ...
    return df[
        (df["Date"] >= str(req_start)) & (df["Date"] <= str(req_end))
    ].reset_index(drop=True)
...
if _sort_by == "lat":
    ...
    _indexed = sorted(
        (x for x in _all_cities if "idx" in x[1]),
        key=lambda x: x[1]["idx"],
        reverse=_sort_desc,
    )
    _rest = sorted(
        (x for x in _all_cities if "idx" not in x[1]),
        key=lambda x: x[0],
        reverse=_sort_desc,
    )
...
for label, key, spec in _rows:
    vals = [
        f"{v:{spec}}" if isinstance(v, (int, float)) else str(v)
        for v in summary_df.loc[key]
    ]
...
fig.suptitle(
    f"Multi-City Temperature & Rainfall Analysis ({_YEAR_LABEL})",
    fontsize=16,
    fontweight="bold",
)
...
for city, df in city_data.items():
    ...
    ax1.plot(
        yearly.index,
        yearly.rolling(5, min_periods=1).mean(),
        label=city,
        color=colors[city],
        linewidth=1.8,
    )
...
bars = ax5.bar(
    cities_list, rain_list, color=[colors[c] for c in cities_list], alpha=0.7
)
...
for city, df in city_data.items():
    ...
    ax6.plot(
        yearly_rain.index,
        yearly_rain.rolling(5, min_periods=1).mean(),
        label=city,
        color=colors[city],
        linewidth=1.8,
    )
...
for city, df in city_data.items():
    ...
    ax7.fill_between(
        yearly.index,
        yearly["Temp Min"].rolling(5, min_periods=1).mean(),
        yearly["Temp Max"].rolling(5, min_periods=1).mean(),
        alpha=0.15,
        color=colors[city],
    )
    ax7.plot(
...
for city, df in city_data.items():
    extreme = (
        ...
        .groupby("Year")
        .size()
        .reindex(df["Year"].unique(), fill_value=0)
    )
    if extreme.sum() > 0:
        ax8.plot(
            extreme.index,
            extreme.rolling(5, min_periods=1).mean(),
            label=city,
            color=colors[city],
            linewidth=1.8,
        )
